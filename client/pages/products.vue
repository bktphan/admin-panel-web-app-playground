...
const handleSave = () => {
  if (!form.name || form.price < 0) return alert('กรุณากรอกชื่อและราคาให้ถูกต้อง')
...
<template>
  <div class="space-y-6 animate-fade-in">
    <div class="flex justify-between items-center">
      <div>
        <h1 class="text-2xl font-bold text-gray-800">จัดการสินค้า (Product Management)</h1>
        <p class="text-gray-500 text-sm">รายการสินค้าทั้งหมดในระบบ</p>
      </div>
      <button 
        @click="openAddModal"
        class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg flex items-center space-x-2 shadow-sm transition-colors"
      >
        <Plus :size="18" />
            ...
            <tr v-for="product in productStore.products" :key="product.id" class="hover:bg-gray-50 transition-colors">
              ...
              <td class="p-4">
                <div class="flex items-center space-x-2">
                  ...
                  <span :class="product.status === 'Active' ? 'text-green-700' : 'text-gray-500'">{{ product.status }}</span>
                </div>
              </td>
    ...
    <div v-if="isModalOpen" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 backdrop-blur-sm p-4">
      <div class="bg-white rounded-2xl w-full max-w-lg shadow-2xl transform transition-all animate-fade-in-up">
